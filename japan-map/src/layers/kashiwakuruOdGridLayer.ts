...
// import type { Feature, FeatureCollection, LineString, Point, Geometry } from "geojson";
...
// // --- hide base meshes when turning this layer ON ---
...
// // ---------------- math (curves) ----------------
// function hash01(s: string) { let h = 2166136261 >>> 0; for (let i = 0; i < s.length; i++) { h ^= s.charCodeAt(i); h = Math.imul(h, 16777619); } return (h >>> 0) / 4294967295; }
...
// }
// function curvedCoords(A: [number, number], B: [number, number], seedKey: string, steps = 16): [number, number][] {
//   const dx = B[0] - A[0], dy = B[1] - A[1], dist = Math.hypot(dx, dy) || 1e-9;
//   const nx = -dy / dist, ny = dx / dist, mid: [number, number] = [(A[0] + B[0]) / 2, (A[1] + B[1]) / 2];
//   const r = hash01(seedKey), sign = r < 0.5 ? -1 : 1, amp = dist * (0.2 + 0.18 * ((r * 2) % 1));
//   const ctrl: [number, number] = [mid[0] + sign * nx * amp, mid[1] + sign * ny * amp];
//   const out: [number, number][] = []; for (let i = 0; i <= steps; i++) out.push(quadPoint(A, ctrl, B, i / steps)); return out;
// }

// // ---------------- grid & stats ----------------
// function computeStatsFromOD(od: FeatureCollection<LineString, any>) {
//   let minLon = +Infinity, minLat = +Infinity, maxLon = -Infinity, maxLat = -Infinity, sumLat = 0, n = 0;
//   for (const f of od.features) {
//     const c = f.geometry?.coordinates; if (!c || c.length < 2) continue;
//     const o = c[0] as [number, number], d = c[c.length - 1] as [number, number];
...
//   const dLat = 0.1 / 110.574;                    // 100 m in degrees lat
//   const dLon = 0.1 / (111.32 * Math.cos(meanLatRad)); // 100 m in degrees lon
//   return { dLat, dLon };
// }
// function idxForPoint(lon: number, lat: number, minLon: number, minLat: number, dLon: number, dLat: number) {
//   return { j: Math.floor((lon - minLon) / dLon), i: Math.floor((lat - minLat) / dLat) };
// }
// function centroidForCell(i: number, j: number, minLon: number, minLat: number, dLon: number, dLat: number) {
//   return [minLon + (j + 0.5) * dLon, minLat + (i + 0.5) * dLat] as [number, number];
// }

// // ---------------- robust time parsing & filter ----------------

// /** Parse hour from variants: 19, "19", "19:00", "1900". Returns NaN if not parseable. */
// function parseHour(v: unknown): number {
//   if (typeof v === "number") return v;
//   if (typeof v !== "string") return NaN;
//   const s = v.trim();
//   // "19:00" -> 19; "19-20" won't be parsed here (handled below)
...
//   return null; // unknown => treat as not matching when filtering is on
// }
...
// /** Keep only features whose props band overlaps the requested band. If requested band is null ⇒ keep all. */
// function filterByTimeBand(od: FeatureCollection<LineString, any>, band: TimeBand): Feature<LineString, any>[] {
//   if (!band) return od.features;
//   const [hs, he] = band;
//   return od.features.filter(f => {
//     const b = extractBand(f.properties);
//     if (!b) return false; // if we can't parse a band, exclude when filtering
//     return bandsOverlap(hs, he, b[0], b[1]);
//   });
// }

// // ---------------- aggregate to 100m cells ----------------

// export type Aggregated = {
//   lines: FeatureCollection<LineString, { vol: number; oI: number; oJ: number; dI: number; dJ: number }>;
//   bubbles: FeatureCollection<Point, { vol: number; i: number; j: number }>;
// };

// function makePairKey(oI: number, oJ: number, dI: number, dJ: number, undirected: boolean) {
//   if (!undirected) return `${oJ}:${oI}>${dJ}:${dI}`;
//   const a = `${oJ}:${oI}`, b = `${dJ}:${dI}`; return a < b ? `${a}>${b}` : `${b}>${a}`;
// }
...
//     if (oI === dI && oJ === dJ) {
//       const k = `${oJ}:${oI}`;
//       selfTotals.set(k, (selfTotals.get(k) || 0) + vol);
//     } else {
//       const k = makePairKey(oI, oJ, dI, dJ, undirected);
//       totals.set(k, (totals.get(k) || 0) + vol);
//     }
//   }

//   const lineFeatures: Feature<LineString, any>[] = [];
//   totals.forEach((vol, key) => {
...
//   return { agg: { lines: linesFC, bubbles: bubblesFC }, meta: { minLon, minLat, maxLon, maxLat, dLon, dLat } };
// }
...
//   [IDS.layerLines, IDS.layerBubbles, IDS.layerGrid].forEach(id => { if (map.getLayer(id)) map.removeLayer(id); });
//   [IDS.srcLines, IDS.srcBubbles, IDS.srcGrid].forEach(id => { if (map.getSource(id)) map.removeSource(id); });
// }

// function addAggregatedODLayers(map: maplibregl.Map, agg: Aggregated, color = "#0f7282") {
//   // Ensure clean slate
...
//     1, 1.2, 6, 3.0, 10, 4.2, 35, 7.2, 100, 10.5, 250, 14.0
//   ];
//   const lineWidthWithZoom: any = [
...
//   map.addLayer({
//     id: IDS.layerLines, type: "line", source: IDS.srcLines,
//     layout: { "line-cap": "round", "line-join": "round", "line-sort-key": ["get", "vol"] },
//     paint: { "line-color": color, "line-opacity": 0.9, "line-blur": 0.2, "line-width": lineWidthWithZoom }
//   });

//   const radiusByVol: any = [
//     "interpolate", ["exponential", 1.4], ["get", "vol"],
//     1, 2, 6, 4, 10, 6, 35, 9, 100, 12, 250, 16
//   ];
//   map.addLayer({
//     id: IDS.layerBubbles, type: "circle", source: IDS.srcBubbles,
//     paint: { "circle-radius": radiusByVol, "circle-color": "#fb8c00", "circle-stroke-color": "#FFFFFF", "circle-stroke-width": 2, "circle-opacity": 0.85 }
//   });
// }

// function add100mGridOverlay(
//   map: maplibregl.Map,
//   meta: { minLon: number; minLat: number; maxLon: number; maxLat: number; dLon: number; dLat: number }
// ) {
...
//       type: "Feature",
//       properties: {},
//       geometry: { type: "LineString", coordinates: [[x, minLat], [x, maxLat]] }
//     });
//   }
//   // horizontals  ✅ fixed i++ (no parentheses)
//   for (let i = 0; i <= nRows; i++) {
//     const y = minLat + i * dLat;
//     lines.push({
//       type: "Feature",
//       properties: {},
//       geometry: { type: "LineString", coordinates: [[minLon, y], [maxLon, y]] }
//     });
//   }

//   const gridFC: FeatureCollection<LineString, any> = { type: "FeatureCollection", features: lines };
//   map.addSource(IDS.srcGrid, { type: "geojson", data: gridFC as any });
//   map.addLayer({
//     id: IDS.layerGrid,
//     type: "line",
//     source: IDS.srcGrid,
//     paint: { "line-color": "#000000", "line-opacity": 0.25, "line-width": 1 },
//   });
// }

// // --------- lightweight OD cache (avoid refetch on every slider move) ---------
...
//   const res = await fetch(url); if (!res.ok) throw new Error(`Fetch failed: ${res.status} ${res.statusText}`);
//   _rawUrl = url; _rawOD = await res.json(); return _rawOD!;
// }
...
//   opts?: { timeBand?: TimeBand; color?: string; showGrid?: boolean; undirected?: boolean; sourceUrl?: string; }
// ) {
//   const { timeBand = null, color = "#0f7282", showGrid = false, undirected = false, sourceUrl = "/data/kashiwa_od_lines.geojson" } = opts || {};
//   setIsLoading(true);
...
//     addAggregatedODLayers(map, agg, color);
//     if (showGrid) add100mGridOverlay(map, meta);
//     setVisible(true);
//   } catch (e) {
...
// // ---------------- PUBLIC: update in-place (no flicker) ----------------
...
//   const { timeBand = null, color = "#0f7282", showGrid = false, undirected = false, sourceUrl = _rawUrl || "/data/kashiwa_od_lines.geojson" } = opts || {};
//   setIsLoading(true);
//   try {
//     const od = await loadRawOD(sourceUrl);
//     const { agg, meta } = aggregateODTo100m(od, timeBand, undirected);

//     // Update data without re-adding layers
...
//     // Grid overlay toggle
//     const hasGrid = !!map.getSource(IDS.srcGrid);
//     if (showGrid && !hasGrid) add100mGridOverlay(map, meta);
//     if (!showGrid && hasGrid) {
//       if (map.getLayer(IDS.layerGrid)) map.removeLayer(IDS.layerGrid);
//       if (map.getSource(IDS.srcGrid)) map.removeSource(IDS.srcGrid);
//     }
...
// kashiwakuruOdGridLayer.ts
// 100m mesh OD with interactions, filters, and darker grid

// kashiwakuruOdGridLayer.ts
// 100m mesh OD layer with robust time filtering, darker grid (major/minor),
// hover tooltips, click-to-focus (発/着/両方), min-volume filter,
// flicker-free updates, and proper TypeScript typings.
...
//   const nx = -dy / dist,
//     ny = dx / dist; // unit normal
//   const mid: [number, number] = [(A[0] + B[0]) / 2, (A[1] + B[1]) / 2];

//   const r = hash01(seedKey);
//   const sign = r < 0.5 ? -1 : 1;
//   const amp = dist * (0.2 + 0.18 * ((r * 2) % 1)); // 0.20–0.38 × chord
//   const ctrl: [number, number] = [mid[0] + sign * nx * amp, mid[1] + sign * ny * amp];
...
// function gridStepDegrees(meanLatDeg: number) {
//   const meanLatRad = (meanLatDeg * Math.PI) / 180;
//   const dLat = 0.1 / 110.574; // 100m in degree-lat
//   const dLon = 0.1 / (111.32 * Math.cos(meanLatRad)); // 100m in degree-lon
//   return { dLat, dLon };
// }
...
// ) {
//   return {
//     j: Math.floor((lon - minLon) / dLon),
//     i: Math.floor((lat - minLat) / dLat),
//   };
// }
...
// // -------------------- Robust time parsing & filtering --------------------
// function parseHour(v: unknown): number {
//   if (typeof v === "number") return v;
//   if (typeof v !== "string") return NaN;
//   const s = v.trim();
//   const m = s.match(/^(\d{1,2})(?::\d{2})?$/); // "19" or "19:00"
//   if (m) return Number(m[1]);
//   if (/^\d{3,4}$/.test(s)) return Number(s.slice(0, s.length - 2)); // "1900" -> 19
//   return NaN;
// }
// function extractBand(props: any): [number, number] | null {
//   if (!props) return null;
...
//   const tb = String(props.timeband ?? props.time ?? "").trim();
//   const m = tb.match(/^(\d{1,2})(?::?\d{2})?\s*-\s*(\d{1,2})(?::?\d{2})?$/); // "19-20" / "19:00-20:00"
//   if (m) {
//     const a = Number(m[1]),
//       b = Number(m[2]);
//     if (!Number.isNaN(a) && !Number.isNaN(b)) return [a, b];
...
// }
// function bandsOverlap(a0: number, a1: number, b0: number, b1: number) {
//   return a0 < b1 && b0 < a1; // half-open overlap
// }
...
//   const linesFC: FeatureCollection<LineString, any> = {
//     type: "FeatureCollection",
//     features: lineFeatures,
//   };
//   const bubblesFC: FeatureCollection<Point, any> = {
//     type: "FeatureCollection",
//     features: bubbleFeatures,
//   };

//   return {
//     agg: { lines: linesFC, bubbles: bubblesFC },
//     meta: { minLon, minLat, maxLon, maxLat, dLon, dLat },
//   };
// }
...
//     map.setFilter(IDS.layerLines, buildLinesFilter());
//   }
//   if (map.getLayer(IDS.layerLinesHL)) {
//     const hlLines = [
//       "all",
//       buildLinesFilter(),
//       ["==", ["get", "pid"], _hoverPid ?? "__none__"],
//     ] as unknown as FilterExpr;
//     map.setFilter(IDS.layerLinesHL, hlLines);
//   }
//   if (map.getLayer(IDS.layerBubbles)) {
//     map.setFilter(IDS.layerBubbles, buildBubblesFilter());
//   }
//   if (map.getLayer(IDS.layerBubblesHL)) {
//     const hlBubs = [
//       "all",
//       buildBubblesFilter(),
//       ["==", ["get", "cid"], _hoverCid ?? "__none__"],
//     ] as unknown as FilterExpr;
...
//   _popup = new maplibregl.Popup({ closeButton: false, closeOnClick: false, offset: 12 });
//   return _popup;
// }
// function clearPopup() {
//   if (_popup) {
...
// // -------------------- Add / Remove / Update --------------------
// function removeAggregatedLayers(map: maplibregl.Map) {
//   // remove HL first, then base, then sources
//   [IDS.layerLinesHL, IDS.layerLines, IDS.layerBubblesHL, IDS.layerBubbles, IDS.layerGrid].forEach(
//     (id) => map.getLayer(id) && map.removeLayer(id)
//   );
//   [IDS.srcLines, IDS.srcBubbles, IDS.srcGrid].forEach(
...
//   map.addSource(IDS.srcLines, {
//     type: "geojson",
//     data: agg.lines as FeatureCollection<Geometry>,
//   });
//   map.addSource(IDS.srcBubbles, {
//     type: "geojson",
//     data: agg.bubbles as FeatureCollection<Geometry>,
//   });
...
//       "line-opacity": 0.9,
//       "line-blur": 0.2,
//       "line-width": lineWidthZoom,
//     },
//   });

//   // Hover highlight (lines) – above base
//   map.addLayer({
//     id: IDS.layerLinesHL,
//     type: "line",
//     source: IDS.srcLines,
...
//   const gridFC: FeatureCollection<LineString, any> = {
//     type: "FeatureCollection",
//     features: lines,
//   };
//   map.addSource(IDS.srcGrid, {
//     type: "geojson",
//     data: gridFC as FeatureCollection<Geometry>,
//   });
//   map.addLayer({
//     id: IDS.layerGrid,
//     type: "line",
...
//       layers: [IDS.layerBubbles, IDS.layerLines],
//     }) as GLFeature[];

//     const fb = feats.find((f: GLFeature) => f.layer.id === IDS.layerBubbles) as
//       | GLFeature
//       | undefined;
//     const fl = feats.find((f: GLFeature) => f.layer.id === IDS.layerLines) as
//       | GLFeature
//       | undefined;

//     if (fb && fb.properties) {
//       _hoverPid = null;
//       _hoverCid = String(fb.properties.cid);
//       applyFilters(map);
//       setCursor(map, true);
...
//         .setLngLat(e.lngLat)
//         .setHTML(
//           `<div style="font:12px/1.3 system-ui,sans-serif"><b>セル</b> ${i}, ${j}<br/><b>自己流動</b> ${vol}</div>`
//         )
//         .addTo(map);
//       return;
//     }
...
//     if (fl && fl.properties) {
//       _hoverCid = null;
//       _hoverPid = String(fl.properties.pid);
//       applyFilters(map);
//       setCursor(map, true);
...
//         .setLngLat(e.lngLat)
//         .setHTML(
//           `<div style="font:12px/1.3 system-ui,sans-serif"><b>OD</b> (${oI},${oJ}) → (${dI},${dJ})<br/><b>量</b> ${vol}</div>`
//         )
//         .addTo(map);
//       return;
//     }
...
//     const { oI, oJ, vol } = f.properties as any;
//     _focusCell = { i: Number(oI), j: Number(oJ) };
//     _focusMode = "out";
//     applyFilters(map);
//     ensurePopup()
//       .setLngLat(e.lngLat)
//       .setHTML(
//         `<div style="font:12px/1.3 system-ui,sans-serif"><b>発</b> セル(${oI},${oJ})<br/><b>この線の量</b> ${vol}<br/><i>（クリック外で解除）</i></div>`
//       )
//       .addTo(map);
//   };
...
//       .setLngLat(e.lngLat)
//       .setHTML(
//         `<div style="font:12px/1.3 system-ui,sans-serif"><b>セル</b> (${i},${j}) にフォーカス<br/><b>自己流動</b> ${vol}<br/><i>（クリック外で解除）</i></div>`
//       )
//       .addTo(map);
//   };
...
//   // register
//   map.on("mousemove", _handlers.move);
...
//   if (_handlers.move) map.off("mousemove", _handlers.move);
//   if (_handlers.leaveLines) map.off("mouseleave", IDS.layerLines, _handlers.leaveLines);
//   if (_handlers.leaveBubbles) map.off("mouseleave", IDS.layerBubbles, _handlers.leaveBubbles);
//   if (_handlers.clickLine) map.off("click", IDS.layerLines, _handlers.clickLine);
//   if (_handlers.clickBubble) map.off("click", IDS.layerBubbles, _handlers.clickBubble);
//   if (_handlers.mapClick) map.off("click", _handlers.mapClick);
...
//     showGrid = false,
//     undirected = false,
//     sourceUrl = "/data/kashiwa_od_lines.geojson",
//     minVolThreshold = 1,
//     focusMode = "all",
//   } = opts || {};
...
//     showGrid = false,
//     undirected = false,
//     sourceUrl = _rawUrl || "/data/kashiwa_od_lines.geojson",
...
//     (map.getSource(IDS.srcLines) as maplibregl.GeoJSONSource).setData(
//       agg.lines as any
//     );
//     (map.getSource(IDS.srcBubbles) as maplibregl.GeoJSONSource).setData(
//       agg.bubbles as any
//     );
...
// kashiwakuruOdGridLayer.ts
// 100m mesh OD layer with:
// - time filtering (robust parsing & overlap)
// - darker grid (major/minor)
// - hover tooltips
// - click-to-focus (発/着/両方)
// - min-volume filter
// - bus stops (O/D/OD) as circles
// - selectable grid cells to focus related flows
//
// Works with MapLibre GL JS (TypeScript).
...
//   // Interactive cells (hit targets + highlights)
...
// // MapLibre helpful type aliases
// type GLFeature = maplibregl.MapGeoJSONFeature;
// type FilterExpr = maplibregl.FilterSpecification;

// // Hide base mesh layers when turning this on
// function hideMeshLayers(map: maplibregl.Map) {
//   [
//     "mesh-1km-fill",
//     "mesh-1km-outline",
...
//   const totals = new Map<string, number>(); // cell→cell totals
//   const selfTotals = new Map<string, number>(); // same-cell
//   const outMap = new Map<string, number>(); // per-cell out volume
//   const inMap = new Map<string, number>(); // per-cell in volume
//   const usedCells = new Set<string>();

//   // Stops build (dedupe by rounded coordinate)
//   type StopRec = { lon: number; lat: number; i: number; j: number; roleMask: number };
//   const stops = new Map<string, StopRec>(); // key = lon.toFixed(6)+","+lat.toFixed(6)
//   const R_O = 1,
//     R_D = 2;
...
//       so.roleMask |= R_O;
//     } else {
//       stops.set(soKey, { lon: o[0], lat: o[1], i: oI, j: oJ, roleMask: R_O });
//     }
//     const sd = stops.get(sdKey);
//     if (sd) {
//       sd.roleMask |= R_D;
//     } else {
//       stops.set(sdKey, { lon: d[0], lat: d[1], i: dI, j: dJ, roleMask: R_D });
//     }
//   }

//   // Lines (curved between cell centroids)
//   const lineFeatures: Feature<LineString, any>[] = [];
//   totals.forEach((vol: number, key: string) => {
//     const [oPart, dPart] = key.split(">");
//     const [oJ, oI] = oPart.split(":").map(Number);
...
//   // Cells (polygons) for used cells only
...
//     features: bubbleFeatures,
//   };
//   const stopsFC: FeatureCollection<Point, any> = {
//     type: "FeatureCollection",
//     features: stopFeatures,
//   };
//   const cellsFC: FeatureCollection<Polygon, any> = {
//     type: "FeatureCollection",
...
//     map.setFilter(IDS.layerBubblesHL, hlBubs);
//   }
//   if (map.getLayer(IDS.layerStopsFocusHL)) {
//     map.setFilter(IDS.layerStopsFocusHL, buildStopsFocusFilter());
//   }
//   if (map.getLayer(IDS.layerCellsHL)) {
//     map.setFilter(IDS.layerCellsHL, buildCellsHLFilter());
//   }
// }
...
//     data: agg.bubbles as FeatureCollection<Geometry>,
//   });
//   map.addSource(IDS.srcStops, {
//     type: "geojson",
//     data: agg.stops as FeatureCollection<Geometry>,
//   });
//   map.addSource(IDS.srcCells, {
//     type: "geojson",
//     data: agg.cells as FeatureCollection<Geometry>,
//   });

//   // --- Cells (interactive, mostly invisible) ---
//   map.addLayer({
//     id: IDS.layerCellsFill,
//     type: "fill",
//     source: IDS.srcCells,
//     paint: {
//       "fill-color": "#6b7280", // gray-500 (just in case)
//       "fill-opacity": 0.0, // hidden, used for hit-testing
//     },
//   });
//   map.addLayer({
...
//   // --- Stops (O / D / OD) ---
//   // Colors: O=emerald, D=violet, OD=slate
//   map.addLayer({
//     id: IDS.layerStopsO,
//     type: "circle",
//     source: IDS.srcStops,
...
//     paint: {
//       "circle-radius": 3,
//       "circle-color": "#4b5563", // gray-600
//       "circle-stroke-color": "#FFFFFF",
//       "circle-stroke-width": 1,
//       "circle-opacity": 0.95,
//     },
//   });
//   // Focused-cell stops highlight (bigger)
//   map.addLayer({
...
// // Visual grid overlay (separate from interactive cells)
// function add100mGridOverlay(
//   map: maplibregl.Map,
//   meta: {
//     minLon: number;
...
//   // General move (lines, bubbles, stops hover tooltips)
//   _handlers.move = (e: maplibregl.MapMouseEvent) => {
//     const feats = map.queryRenderedFeatures(e.point, {
//       layers: [
//         IDS.layerBubbles,
...
//         .setLngLat(e.lngLat)
//         .setHTML(
//           `<div style="font:12px/1.3 system-ui,sans-serif"><b>停留所</b> (${i},${j})<br/><b>役割</b> ${role}</div>`
//         )
//         .addTo(map);
//       return;
//     }
...
//     // nothing hovered for these layers
//     _hoverPid = _hoverCid = null;
//     applyFilters(map);
//     setCursor(map, false);
//     clearPopup();
...
//   // Cell hover (separate so it doesn't compete with lines/stops)
...
//   _handlers.mapClick = (e: maplibregl.MapMouseEvent) => {
//     // clicking empty space clears focus & cell hover
//     const feats = map.queryRenderedFeatures(e.point, {
//       layers: [
//         IDS.layerCellsFill,
//         IDS.layerBubbles,
...
//     // also track cell hover state for highlight
//     const feats = map.queryRenderedFeatures(e.point, { layers: [IDS.layerCellsFill] }) as GLFeature[];
//     if (feats.length && feats[0].properties) {
//       const { i, j } = feats[0].properties as any;
//       _hoverCell = { i: Number(i), j: Number(j) };
//       applyFilters(map);
//       setCursor(map, true);
//       const { outVol, inVol, total } = feats[0].properties as any;
//       ensurePopup()
//         .setLngLat(e.lngLat)
//         .setHTML(
//           `<div style="font:12px/1.3 system-ui,sans-serif"><b>セル</b> (${i},${j})<br/>発: ${outVol} / 着: ${inVol} / 合計: ${total}</div>`
//         )
//         .addTo(map);
//       return;
//     }
//     // Otherwise, defer to generic hover over lines/bubbles/stops
//     _handlers.move!(e);
//   });
...
//   // The generic mousemove is inline above; no direct unbind needed for that wrapper.
...
//     } else {
//       map.addSource(IDS.srcCells, { type: "geojson", data: agg.cells as any });
//     }

//     // ensure paint updates
//     map.setPaintProperty(IDS.layerLines, "line-color", color);
...
// 100m mesh OD layer with:
// - time filtering (robust parsing & overlap)
// - darker, selectable grid (all cells + used cells)
// - hover tooltips (shadow + rounded)
// - click-to-focus (発/着/両方)
// - min-volume filter
// - bus stops (O=green, D=red, OD=gray) with zoom scaling
// - show/hide stops via options
//
// MapLibre GL JS (TypeScript)
...
//   if (map.getLayer(IDS.layerLines)) map.setFilter(IDS.layerLines, buildLinesFilter());
//   if (map.getLayer(IDS.layerLinesHL)) {
//     const hlLines = ["all", buildLinesFilter(), ["==", ["get", "pid"], _hoverPid ?? "__none__"]] as unknown as FilterExpr;
//     map.setFilter(IDS.layerLinesHL, hlLines);
//   }
//   if (map.getLayer(IDS.layerBubbles)) map.setFilter(IDS.layerBubbles, buildBubblesFilter());
//   if (map.getLayer(IDS.layerBubblesHL)) {
...
//   map.getCanvas().style.cursor = pointer ? "pointer" : "";
// }
// const POPUP_INNER_STYLE =
//   "background:#fff;padding:8px 10px;border-radius:8px;box-shadow:0 10px 30px rgba(0,0,0,.2);font:12px/1.35 system-ui,sans-serif;color:#111;";
// function ensurePopup() {
//   if (_popup) return _popup;
//   _popup = new maplibregl.Popup({ closeButton: false, closeOnClick: false, offset: 12, className: "ai-popup" });
//   return _popup;
// }
// function clearPopup() {
//   if (_popup) {
...
//       paint: {
//         "fill-color": "#111827",
//         "fill-opacity": 0.06, // visible but subtle
//       },
//     });
//     map.addLayer({
...
//       paint: {
//         "line-color": "#111827",
//         "line-opacity": 0.35, // darker outlines
//         "line-width": 1.4,
//       },
//     });
//   }
...
//     paint: {
//       "fill-color": "#111827",
//       "fill-opacity": 0.10, // darker than all-cells
//     },
//   });
//   map.addLayer({
//     id: IDS.layerCellsOutline,
...
//   // Also ensure the "all cells" sources/layers exist so **any** cell can be selected
...
//       },
//     });
//     // If highlight layer already exists and was using used-cells source, repoint it:
//     if (map.getLayer(IDS.layerCellsHL)) {
//       map.removeLayer(IDS.layerCellsHL);
//     }
//     map.addLayer({
//       id: IDS.layerCellsHL,
//       type: "fill",
...
//       ensurePopup()
//         .setLngLat(e.lngLat)
//         .setHTML(`<div style="${POPUP_INNER_STYLE}"><b>停留所</b> (${i},${j})<br/><b>役割</b> ${role}</div>`)
//         .addTo(map);
//       return;
//     }
...
//       ensurePopup()
//         .setLngLat(e.lngLat)
//         .setHTML(`<div style="${POPUP_INNER_STYLE}"><b>セル</b> ${i}, ${j}<br/><b>自己流動</b> ${vol}</div>`)
//         .addTo(map);
//       return;
//     }
...
//         .setLngLat(e.lngLat)
//         .setHTML(
//           `<div style="${POPUP_INNER_STYLE}"><b>OD</b> (${oI},${oJ}) → (${dI},${dJ})<br/><b>量</b> ${vol}</div>`
//         )
//         .addTo(map);
//       return;
//     }
...
//   // Cell hover via ALL-cells (preferred), else used-cells
//   _handlers.leaveCells = (_e: maplibregl.MapLayerMouseEvent) => {
//     _hoverCell = null;
//     applyFilters(map);
//     setCursor(map, false);
...
//     ensurePopup()
//       .setLngLat(e.lngLat)
//       .setHTML(`<div style="${POPUP_INNER_STYLE}"><b>発</b> セル(${oI},${oJ})<br/><b>この線の量</b> ${vol}<br/><i>（クリック外で解除）</i></div>`)
//       .addTo(map);
//   };
//   _handlers.clickBubble = (e: maplibregl.MapLayerMouseEvent) => {
//     const f = e.features?.[0] as GLFeature | undefined;
...
//     ensurePopup()
//       .setLngLat(e.lngLat)
//       .setHTML(`<div style="${POPUP_INNER_STYLE}"><b>セル</b> (${i},${j}) にフォーカス<br/><b>自己流動</b> ${vol}<br/><i>（クリック外で解除）</i></div>`)
//       .addTo(map);
//   };
...
//   // Register combined mousemove to prefer ALL-cells first (if present)
//   map.on("mousemove", (e: maplibregl.MapMouseEvent) => {
...
//       ensurePopup()
//         .setLngLat(e.lngLat)
//         .setHTML(`<div style="${POPUP_INNER_STYLE}"><b>セル</b> (${i},${j})<br/>発: ${outVol} / 着: ${inVol} / 合計: ${total}</div>`)
//         .addTo(map);
//       return;
//     }
//     // Else fallback to lines/bubbles/stops hover
//     _handlers.move!(e);
//   });
...
//     } else {
//       // remove ALL-cells layers if present; keep used-cells layers
...
//       // make sure highlight points to used cells source
//       if (map.getLayer(IDS.layerCellsHL)) map.removeLayer(IDS.layerCellsHL);
//       map.addLayer({
//         id: IDS.layerCellsHL,
//         type: "fill",
...
// - time filtering (robust parsing & overlap)
// - darker, selectable grid (all cells + used cells)
// - hover tooltips (className: "ai-popup")
// - click-to-inspect ONE OD line (isolation)
// - click-to-focus cell (発/着/両方 still supported via filters)
// - min-volume filter
// - bus stops (O=green, D=red, OD=gray) with zoom scaling; togglable via showStops
//
// MapLibre GL JS (TypeScript)
